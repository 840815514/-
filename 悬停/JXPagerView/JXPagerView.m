...
- (UIView *)tableView:(UITableView *)tableView viewForHeaderInSection:(NSInteger)section {
    JXPagerView *jView = (JXPagerView *)[self.delegate viewForPinSectionHeaderInPagerView:self];
    UIView *hView = [[UIView alloc]initWithFrame:CGRectMake(0, 0, [UIScreen mainScreen].bounds.size.width, jView.frame.size.height)];
    [hView addSubview:jView];
    
    UIButton *addBtn = [[UIButton alloc]initWithFrame:CGRectMake([UIScreen mainScreen].bounds.size.width-40, 2.5, 40, 40)];
    [addBtn setImage:[UIImage imageNamed:@"添加"] forState:UIControlStateNormal];
    [hView addSubview:addBtn];
    
    UIImageView *lineImv = [[UIImageView alloc]initWithFrame:CGRectMake(0, 45, [UIScreen mainScreen].bounds.size.width, 1)];
    lineImv.backgroundColor = [UIColor colorWithRed:229/255.0 green:229/255.0 blue:229/255.0 alpha:1];
    [hView addSubview:lineImv];
